...
// Bumpアロケータ (Linearアロケータ) アルゴリズムによるアロケータの実装
...
void map_page(uint32_t *table1, uint32_t vaddr, paddr_t paddr, uint32_t flags)
{
    ...
    // 22 = vpn0(10bit) + offset(12bit)
    // 0x3ff = 0b11_1111_1111 (最上位10bit全てON)
    ...
    uint32_t offset_bits = (table1[vpn1] >> 10);
    uint32_t *table0 = (uint32_t *)(offset_bits * PAGE_SIZE);
                                           ...
                                           uint32_t *next_sp)
{
    __asm__ __volatile__(
        ...
        "addi sp, sp, -13 * 4\n" // 13個分のレジスタ分 Stack を下に伸ばす。レジスタ1つあたり32bit=4bytes.
        "sw ra,  0  * 4(sp)\n"   // ra = Return Address レジスタ
        "sw s0,  1  * 4(sp)\n"
        "sw s1,  2  * 4(sp)\n"
        "sw s2,  3  * 4(sp)\n" // s2 の値を (3*4)+sp のアドレスに書き込む
        "sw s3,  4  * 4(sp)\n"
        "sw s4,  5  * 4(sp)\n"
        "sw s5,  6  * 4(sp)\n"
        "sw s6,  7  * 4(sp)\n"
        ...
        // スタックポインタの切り替え
        "sw sp, (a0)\n" // sw = Store Word, 32bitレジスタの値をメモリに書き込む (*a0 = sp) a0は0番目の引数(=prev_sp)
        "lw sp, (a1)\n" // lw = Load Word, a1(next_sp) から sp を復元 (sp = *next_sp)
...
struct process *create_process(const void *image, size_t image_size)
{
    ...
    // swtich_context() で復帰できるように、Stackに呼び出し先保存レジスタを積む
...
struct sbiret sbi_call(long arg0, long arg1, long arg2, long arg3, long arg4, long arg5, long fid, long eid)
{
    ...
    register long a5 __asm__("a5") = arg5; // Extension ID
    register long a6 __asm__("a6") = fid;  // Function ID
    register long a7 __asm__("a7") = eid;

    // ecall 命令でひとつ下のレイヤの機能を呼び出す
    // Kernel -> SBI, App -> Kernel
...
void virtio_blk_init(void)
{
    ...
    // ACKNOWLEDGEステータスビットを設定: デバイスを認識した
    virtio_reg_fetch_and_or32(VIRTIO_REG_DEVICE_STATUS, VIRTIO_STATUS_ACK);
    // DRIVERステータスビットを設定: デバイスの使い方を知っている
    virtio_reg_fetch_and_or32(VIRTIO_REG_DEVICE_STATUS, VIRTIO_STATUS_DRIVER);
    // ページサイズを設定: 4KBを使用。Page Frame Number の計算に使われる
    virtio_reg_write32(VIRTIO_REG_PAGE_SIZE, PAGE_SIZE);
    // ディスク読み書き用のキューを初期化
    blk_request_vq = virtq_init(0);
    // DRIVER_OKステータスビットを設定: デバイスが使用可能になった
...
// desc_index は新しいリクエストの先頭ディスクリプタのインデックス。
...
void read_write_disk(void *buf, unsigned sector, int is_write)
{
    ...
    // virtqueue のディスクリプタを構築する (3つのディスクリプタを使う)
...
void fs_init(void)
{
    ...
    for (int i = 0; i < FILES_MAX; ++i)
    {
        ...
        offset += align_up(sizeof(struct tar_header) + filesz, SECTOR_SIZE);
    }
}
...
__attribute__((naked)) // 関数本文の前後に余計なコード(Prologue, Epilogue)を生成しない
void boot(void)        // Kernel のエントリーポイント
{
    __asm__ __volatile__(
        "mv sp, %[stack_top]\n"
        "j kernel_main\n"
